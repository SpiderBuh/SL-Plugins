...
public class Track : ICustomCommand
{
    public string Command => "track";

    public string[] Aliases => null;

    public string Description => "Sets the track of a speaker toy.";
    ...
    public bool Execute(ArraySegment<string> arguments, ICommandSender sender, out string response)
    {
        ...
        if (!int.TryParse(arguments.At(1), out var trackNum))
        {
            response = "Could not parse track number";
            return false;
        }
        
        speakerToy.CurrentPosition = trackNum;
        
        response = $"THe thingy hgas been set to track {trackNum}";
        return true;
    }
}