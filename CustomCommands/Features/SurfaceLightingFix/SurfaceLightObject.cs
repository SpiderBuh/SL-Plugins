...
namespace CustomCommands.Features.SurfaceLightFix
{
	public class SurfaceLightObject : MonoBehaviour
	{
		...
		private void Start()
		{
			Logger.Info("Surface light starting");
			controller = RoomLightController.Instances.Find(x => x.Room.Name == RoomName.Outside);
			if (controller == default)
			{
				Logger.Warn("Surface light controller not found, will try again in 10s");
