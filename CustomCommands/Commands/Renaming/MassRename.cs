	...
	public class MassRename : ICustomCommand
	{
		...
		public bool Execute(ArraySegment<string> arguments, ICommandSender sender, out string response)
		{
			try
			{
				...
				int n = LabApi.Features.Wrappers.Player.Count;
				if (prfxN)
				{
					n = f('N', prefix).FirstOrDefault();
					if (prfxNOT)
						n = LabApi.Features.Wrappers.Player.Count - n;
				}
				...
				if (prfxAsc || prfxDesc)
				{
					players.Sort(Comparer<LabApi.Features.Wrappers.Player>.Create((x, y) => x.Nickname.CompareTo(y.Nickname)));
