...
namespace CustomCommands.Features.SCPs.DamageAnnouncements
{
	public class AnnouncementEvents
	{
		...
		{
			if (!CustomCommandsPlugin.Config.EnableDamageAnnouncements || !AnnouncementManager.ScpDamage.Any())
				return;
			...
			Logger.Info($"Damage values: {dmg.Count()}");
			...
			Logger.Info($"Damage count: {str.Count}");
